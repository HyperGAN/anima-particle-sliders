...
def main():
    ...
    provenance=dict(repository='https://github.com/mikkel/anima-concept-sliders',commit=commit,
    ...
    # Refuse to overwrite an unrelated pre-existing release.
    existing=api.list_repo_files(REPO,repo_type='model')
    assert set(existing)<={'.gitattributes'},'Reconcile existing release before updating'
    result=api.upload_folder(repo_id=REPO,repo_type='model',folder_path=folder,
        parent_commit=before.sha,
        commit_message='Release Candlelit and Moonlit: particles, distilled LoRAs, strengths 0–5 and reproduction source')
    revision=result.oid
    ...
    report=dict(repo=REPO,commit=revision,source_commit=commit,verified_files=checked,readbacks=3)
