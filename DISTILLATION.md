...
`release_tools/distill.py` reproduces the fitting, exports both native and ComfyUI tensor names, and renders the comparisons. The exporter does not fuse Q/K/V because ComfyUI Anima exposes separate projections. `release_tools/verify_comfy.py` checks all 224 mappings against actual ComfyUI modules and verifies native/ComfyUI matrix equality.
